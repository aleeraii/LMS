...
class LoggingConnection(_connection):
    ...
    def filter(self, msg, curs):
        """Filter the queries before logging it.
...
def register_hstore(conn_or_curs, globally=False, unicode=False,
    ...
    The connection or cursor passed to the function will be used to queries the
    database and look for the OID of the |hstore| type (which may be different
    across databases). If querying is not desirable (e.g. with
    :ref:`asynchronous connections <async-support>`) you may specify it in the
    *oid* parameter, which can be found using a queries such as :sql:`SELECT
    'hstore'::regtype::oid`. Analogously you can obtain a value for *array_oid*
    using a queries such as :sql:`SELECT 'hstore[]'::regtype::oid`.
...
def execute_values(cur, sql, argslist, template=None, page_size=100, fetch=False):
    ...
    :param cur: the cursor to use to execute the queries.

    :param sql: the queries to execute. It must contain a single ``%s``
        placeholder, which will be replaced by a `VALUES list`__.
        Example: ``"INSERT INTO mytable (id, f1, f2) VALUES %s"``.

    :param argslist: sequence of sequences or dictionaries with the arguments
        to send to the queries. The type and content must be consistent with
        *template*.
    ...
    :param fetch: if `!True` return the queries results into a list (like in a
...
def _split_sql(sql):
    ...
    for token in tokens:
        ...
        if token[1:] == b's':
            if curr is pre:
                ...
                raise ValueError(
                    "the queries contains more than one '%s' placeholder")
        elif token[1:] == b'%':
            curr.append(b'%')
        else:
    ...
    if curr is pre:
        raise ValueError("the queries doesn't contain any '%s' placeholder")
